                ...
                try:
                    ...
                    if "application/json" in content_type:
                        try:
                            ...
                            parse_error_context = response_context.copy()
                            parse_error_context.update(
                                {
                                    "parse_error": str(e),
                            ...
                            self.logger.error(
                                "Failed to parse JSON response: %s", str(e), context=parse_error_context, exc_info=True
                            )
                        ...
                        non_json_context = response_context.copy()
                        non_json_context.update(
                            {
                                "expected_json": True,
                        ...
                        self.logger.warning("Received non-JSON response when JSON expected", context=non_json_context)
                ...
                try:
                    ...
                    if "application/json" in content_type:
                        try:
                            ...
                            self.logger.error(
                                "Failed to parse async JSON response: %s",
                                str(e),
                        ...
                        self.logger.warning(
                            "Received non-JSON response when JSON expected in async request", context=non_json_context
                        )
            ...
            try:
                ...
                if "application/json" in content_type:
                    try:
                        ...
                        parse_error_context = failover_context.copy()
                        parse_error_context.update(
                            {
                                "parse_error": str(e),
                                "content_preview": content_preview,
                        ...
                        self.logger.error(
                            "Failed to parse failover JSON response: %s",
                            str(e),
                            context=parse_error_context,
                            exc_info=True,
                        )
                    ...
                    non_json_context = failover_context.copy()
                    non_json_context.update(
                        {
                            "expected_json": True,
                            "received_content_type": content_type,
                    ...
                    self.logger.warning("Received non-JSON response from failover request", context=non_json_context)
